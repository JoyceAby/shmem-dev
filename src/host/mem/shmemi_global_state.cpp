...
{
    SHMEM_CHECK(load_hal_library());

    SHMEM_CHECK(halMemAddressReserveFunc(&device_ptr_, GLOBAL_STATE_SIZE, 0, (void *)(SVM_END_ADDR - GLOBAL_STATE_SIZE), 1));
    ...
    SHMEM_CHECK(halMemCreateFunc(&alloc_handle, GLOBAL_STATE_SIZE, &memprop, 0));

    SHMEM_CHECK(halMemMapFunc(device_ptr_, GLOBAL_STATE_SIZE, 0, alloc_handle, 0));
...
{
    SHMEM_CHECK(halMemUnmapFunc(device_ptr_));

    SHMEM_CHECK(halMemReleaseFunc(alloc_handle));

    SHMEM_CHECK(halMemAddressFreeFunc(device_ptr_));
